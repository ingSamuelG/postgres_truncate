...
POSG_TRUN_SIZE = 50000
CREATE_TRUNCATED_DB = False
CREATE_SQL_DUMB = True


if CREATE_TRUNCATED_DB == False and CREATE_SQL_DUMB == False:
        print("You are running the script without any output change the constants to true to produce an output")
        ...
        Postql_iso_temp = PostSql.PostSqlIsolated(POSG_HOST, POSG_USER, POSG_PASS,"dev_{}".format(POSG_DB))
        Postql_iso_temp.close_conections()
                ...
                if parents != None:
                        ...
                        for parent in parents:
                                ...
                                if parent not in parent_created:
                                        Postql.drop_existing_table('xyxy_{}'.format(parent))
                                ...
                                if not ids_seeded.get(parent):
                                        ...
                                        if parent not in POSG_TRUNCATE_TO_0_TABLES:
                                                Postql.insert_from_old(source_table_rows, 'xyxy_{}'.format(parent))
                                                tables_added.append('xyxy_{}'.format(parent))
                        ...
                        if ids_seeded.get(table_child_key):
                                ...
                                Postql.drop_existing_table('xyxy_{}'.format(table_child_key))
                                ...
                                if table_child_key not in POSG_TRUNCATE_TO_0_TABLES:
                                        ...
                                        tables_added.append('xyxy_{}'.format(table_child_key))
                else:
                        Postql.drop_existing_table('xyxy_{}'.format(table_child_key))
                        Postql.copy_tables_from_main(table_child_key)
                        print("Cloning table {}".format(table_child_key))
                        if table_child_key not in POSG_TRUNCATE_TO_0_TABLES:
                                ...
                                Postql.insert_from_old(source_table_rows, 'xyxy_{}'.format(table_child_key))
                                tables_added.append('xyxy_{}'.format(table_child_key))

        nt_table_names = [results[0] for results in Postql.get_table_names() if re.search('^xyxy_*', results[0])]

        last_tables = [table_name.replace("xyxy_", "" ) for table_name in nt_table_names]

        # os.environ["PGPASSWORD"] = POSG_PASS
        # # command = 'pg_dump -U {} -w  -t "^xyxy_*" -f ./dev_x3.sql {}'.format(POSG_USER, POSG_DB)
        # command = 'pg_dump -U {} -h {} -w  -t "^xyxy_*" {} > ./dev_x3.sql '.format(POSG_USER, POSG_HOST ,POSG_DB)
        ...
        os.environ["PGPASSWORD"] = POSG_PASS
        command = "pg_dump -U {} -h {} -w -t \"^xyxy_*\" {} | psql -U {} -h {} -w -d dev_{}".format(POSG_USER, POSG_HOST ,POSG_DB,POSG_USER,POSG_HOST, POSG_DB.lower() )
        c = delegator.run(command)
        # time.sleep(10)
        ...
        Postql_iso = PostSql.PostSqlConnect(POSG_HOST, POSG_USER, POSG_PASS,"dev_{}".format(POSG_DB.lower()), POSG_END)

        for table_name in last_tables:
                Postql_iso.copy_tables_from_main_wo_pre(table_name)
                Postql_iso.drop_existing_table("xyxy_{}".format(table_name))

        Postql_iso.commit_all_transactions()
        Postql_iso.close_conections()

        if not CREATE_TRUNCATED_DB:
                Postql_iso_temp = PostSql.PostSqlIsolatedDrop(POSG_HOST, POSG_USER, POSG_PASS,"dev_{}".format(POSG_DB.lower()))
                print("Skipped database")
        else:
                print("Database created as dev_{}".format(POSG_DB.lower()))
        
        if CREATE_SQL_DUMB:
                command = 'pg_dump -U {} -h {} -w  -d dev_{} > ./dev_x3.sql '.format(POSG_USER, POSG_HOST ,POSG_DB.lower())
                c = delegator.run(command)
                print("SQL dumb created")
